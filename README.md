...
あらかじめ画像を登録・チェックしておくと、紐づけた機能ページを開いたときに
自動でその画像が読み込まれる機能。

- 1枚のWK画像は基本的に1つの機能ページ（`targetPath`）に紐づく。同じ機能ページに
  対して「自動選択する」がオンの画像は常に1枚だけになるよう排他制御している
  （`src/lib/wkImageStore.js` の `setCheckedInList`）。
- 機能ページに既に画像が読み込まれている状態でWK画像が届いた場合は、確認ダイアログを
  挟んでから上書きする（`src/hooks/useWkAutoLoad.js`）。
- 自動選択された画像は、実際にページへ読み込まれた時点でチェックが自動的に外れる
  （単発の受け渡し用途のため。一覧からは消えず、再度チェックすれば再利用できる）。
