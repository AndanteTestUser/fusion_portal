...
既定では自動ポーリングはオフで、ウェルカム画面・設定画面の「🔄 取り込む」ボタンを押した
ときだけ取得しにいく。アプリを開いている間ずっとバックグラウンドで自動取得したい場合は、
同じ設定画面の「アプリを開いている間、自動的にバックグラウンドで取り込む」をオンにする
（60秒間隔でポーリングする）。いずれの場合もURL・シークレットが未設定ならネットワーク
通信は一切発生しない。
